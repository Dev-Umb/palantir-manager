...
export default function AiIndex({ conversations: initialConversations }) {
    ...
    async function sendMessage(event, retryMessage = null, retryParentId = null) {
    ...
    async function handleProposalAction(runId, proposalId, action) {
        setError('');
        const response = await fetch(`/ai/runs/${runId}/proposals/${proposalId}/${action}`, {
            method: 'POST',
            headers: jsonHeaders(),
            body: JSON.stringify({}),
        });
        const data = await response.json().catch(() => ({}));
        if (data.run) {
            updateRun(runId, () => normalizeRun(data.run));
        }
        if (!response.ok) {
            throw new Error(data.message || '无法处理该确认卡片。');
        }
        return data;
    }
                ...
                <div className="ai-progress-events">
                    {(run.activity || []).map((item, index) => (
                        <div key={`${item.seq || index}-${item.type}`}>
                            ...
                            <p>{businessText(item.label) || '正在处理'}</p>
    ...
    return (
        <details className="ai-sources-v2">
            ...
            <div>{items.map((source, index) => (
                <span key={`${source.object_key}-${index}`}>{businessText(source.object_label || source.object_key)} · {source.record_count} 条</span>
            ))}</div>
        </details>
    );
}
