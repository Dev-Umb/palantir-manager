        ...
        <div className="timebook">
            ...
            {(can('create') || editing && can('update')) && <section className="surface timebook-section">
                ...
                <form onSubmit={save}><fieldset disabled={busy} className="timebook-form">
                    ...
                    <label>加班（小时）<input type="number" min="0" max="24" step="any" required value={entry.overtime} onChange={set('overtime')} /></label>
                ...
                <div className="timebook-table" tabIndex={0} role="region" aria-label={view === 'records' ? '记工明细表' : '人员汇总表'} aria-busy={loading}>
                    {view === 'records' ? <table><thead><tr>{['日期', '姓名', '工日（天）', '加班（小时）', '工作项目', '备注', '操作'].map((label) => <th key={label}>{label}</th>)}</tr></thead>
                        <tbody>{data.records.map((row) => <tr key={row.id}>
                            <td>{row.day}</td><td><button disabled={busy} onClick={() => worker(row)}>{row.name}</button></td><td>{display(row.days)}</td><td>{display(row.overtime)}</td><td>{row.project || '—'}</td><td>{row.note || '—'}</td>
                            <td className="timebook-actions">{can('update') && <button disabled={busy} onClick={() => edit(row)}>修改</button>}{can('audit') && <button disabled={busy} onClick={() => showHistory(row)}>留痕</button>}{can('delete') && <button disabled={busy} onClick={() => setDeleting(row)}>删除</button>}</td>
                        ...
                        : <table><thead><tr>{['姓名', '累计工日（天）', '加班（小时）', '记录数', '首次日期', '最近日期'].map((label) => <th key={label}>{label}</th>)}</tr></thead>
                            <tbody>{data.summary.map((row) => <tr key={row.worker_id}><td><button disabled={busy} onClick={() => { worker(row); setView('records'); }}>{row.name}</button></td><td>{display(row.days)}</td><td>{display(row.overtime)}</td><td>{row.count}</td><td>{row.first_day}</td><td>{row.last_day}</td></tr>)}{!data.summary.length && <tr><td colSpan={6}>当前范围没有人员汇总。</td></tr>}</tbody></table>}
                </div>
