...
class ProcessFeishuInboundEvent implements ShouldBeUnique, ShouldQueue
{
    ...
    public function handle(CreateFeishuAiRun $runs, FeishuProcessingReaction $reaction): void
    {
        ...
        if (! app(FeishuRollout::class)->allowsUser($binding->user_id)) {
            $inbound->update([
                'status' => 'ignored',
        ...
        if ($messageType === 'file') {
            ...
            $this->completeOrQueueCard($inbound, $card, $reaction);
    ...
    private function sendCard(FeishuInboundEvent $event, FeishuClient $client, array $card): array
    {
        ...
        if ($chatType === 'group' && ! app(FeishuRollout::class)->isRestricted()) {
    ...
    /** @param array<string, mixed> $card */
    private function completeOrQueueCard(
        FeishuInboundEvent $event,
        array $card,
        FeishuProcessingReaction $reaction,
    ): void {
        try {
            $reply = $this->sendCard($event, app(FeishuClient::class), $card);
        } catch (FeishuOutboundCooldownException $exception) {
            $payload = $event->payload;
            data_set($payload, '_palantir.pending_reply_card', $card);
            $event->update([
                'status' => 'reply_pending',
                'payload' => $payload,
                'error' => null,
            ]);
            SendFeishuWorkflowReply::dispatch($event->id)
