...
class DeliverFeishuNotification implements ShouldBeUnique, ShouldQueue
{
    ...
    public function handle(FeishuClient $client, FeishuMessageRenderer $renderer): void
    {
        ...
        $rollout = app(FeishuRollout::class);
        $destinationUserId = $rollout->reminderDestinationUserId($delivery->user_id);
        if ($destinationUserId === null) {
            NotificationDelivery::query()
                ...
                ->update([
                    ...
                    'last_error' => 'rollout_recipient_restricted',
                ]);

            return;
        }

        $redirectsReminderDeliveries = $rollout->redirectsReminderDeliveries();
        Cache::lock("feishu:notification-delivery:user:{$destinationUserId}", 30)
            ->block(10, function () use ($client, $delivery, $destinationUserId, $renderer, $redirectsReminderDeliveries): void {
                $deliveries = NotificationDelivery::query()
                    ->when(! $redirectsReminderDeliveries, fn ($query) => $query->where('user_id', $delivery->user_id))
                    ->where('channel', 'feishu')
                    ->where('status', NotificationDelivery::STATUS_PENDING)
                    ->orderBy('id')
                    ->get();
                ...
                $binding = FeishuUserBinding::active()->where('user_id', $destinationUserId)->first();
                if (! $binding) {
                    if ($redirectsReminderDeliveries) {
                        throw new \RuntimeException('feishu_rollout_recipient_not_bound');
                    }
