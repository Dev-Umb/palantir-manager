...
class SyncProjectContracts
{
    ...
    public function appendStoredAttachment(
        ...
        abort_unless(in_array($source, ['feishu', 'ai'], true), 422);
        ...
        DB::transaction(function () use ($project, $contract, $field, $attachment, $user, $source): void {
            $project = ObjectRecord::query()
                ->whereKey($project->id)
                ->whereRelation('businessObject', 'key', 'project')
                ->lockForUpdate()
            ...
            AuditLog::create([
                'user_id' => $user->id,
                'action' => "object.update.project_contract.{$source}_attachment",
                'subject_type' => 'contract',
                'subject_id' => $contract->id,
                'payload' => [
                    'code' => $contract->code,
