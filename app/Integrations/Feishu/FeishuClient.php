...
class FeishuClient
{
    public function __construct(private FeishuOutboundMessageGuard $outboundMessages) {}
    ...
    private function sendMessage(
        ...
        return $this->outboundMessages->send($receiveIdType, $receiveId, function () use ($content, $messageType, $receiveId, $receiveIdType): array {
            $response = $this->withTenantAccessToken(fn (string $token): Response => $this->request()->withToken($token)
