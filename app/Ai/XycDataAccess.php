...
class XycDataAccess
{
    public function __construct(
        ...
        private AiQueryProvenance $provenance,
    ) {}
    ...
    public function queryRecords(User $user, array $input): array
    {
        ...
        $result['provenance'] = $this->provenance->make(
            'query_object_records', $object->key, $input, $recordIds, $result,
        );
        ...
        $this->audit($user, 'ai.tool.query_records', $object->key, null, [
            ...
            'record_ids_hash' => $result['provenance']['record_ids_hash'],
            'result_hash' => $result['provenance']['result_hash'],
    ...
    private function listPostgres(BusinessObject $object, Builder|Relation $query, array $input, User $user): array
    {
    ...
    private function fieldDefinitions(BusinessObject $object): array
    {
        ...
        return [
            ...
            ...$objectFields->map(fn (array $field) => [
                'key' => $field['key'],
                'label' => $field['label'],
                'type' => $field['type'],
                'target' => $field['target'] ?? null,
