...
### 10. Markdown 按语义转换为飞书原生卡片元素

成功回答不再整体塞入单个 `lark_md`。渲染器按标题、普通段落、项目列表和 Markdown 表格拆分元素；表格逐行转换为带字段的项目块，标题标记被消费而不是原样显示。无法识别的文本仍以受限 `lark_md` 段落保留，因此回答内容不丢失，失败回答继续使用 text 消息。

### 11. 新建导出使用受控 lark-cli 边界

生产安装 `lark-cli` 并配置与当前机器人相同的独立应用身份，不执行 OpenClaw/Hermes channel bind。Laravel 通过参数数组启动 CLI，不经过 shell；文档正文经标准输入传递，表格 JSON 作为独立参数传递并受行数、列数和字节数上限约束。CLI 输出只接受 JSON envelope，错误只保留类型、代码和脱敏提示。

飞书 agent 新增两个专用工具：新建云文档导出和新建电子表格导出。模型只提供对象、字段、过滤、排序、行数上限和标题；工具内部再次调用 `XycDataAccess`，以当前绑定的 Palantir 用户执行查询，并从系统字段定义生成列名。模型不能传入任意伪造行，也不能提供既有飞书 token，因此工具无法读取、覆盖、移动或删除已有文件。

新文件由机器人应用身份创建，随后以新文件 token 只向发起人的绑定 `open_id` 增加只读协作者权限，成功后在原会话回复中展示标题、类型、行数和 URL。该模式不访问用户个人云空间，因此不需要用户 OAuth；若未来需要操作个人已有文件，必须另行设计用户授权、token 生命周期和撤销流程。
...
### 12. 飞书合同文件采用先耐久暂存、后确定性绑定

文件消息不进入模型提示词。入站任务先检查会话、绑定和合同维护权限，再通过消息资源 API 下载原始二进制；仅接受 PDF/JPEG/PNG 和受限大小。存储服务写入配置的私有附件盘后复核对象存在、字节数并记录 SHA-256、原始文件名、MIME、消息 ID、发送人和状态。生产附件盘使用独立私有 TOS 桶；本地测试使用隔离 fake disk。旧 `local` 路径继续由下载控制器回退读取。

用户随后发送固定格式的绑定指令，服务端只对项目编号与合同编号做标准化后的精确等值查询，并要求两个结果均唯一且合同确属该项目。任何缺失、重复、冲突或越权都返回无写入确认卡，不让模型选择候选项。成功路径复用项目合同同步动作追加附件并写审计，然后把暂存记录标为已绑定；既有附件数组原样保留。

### 13. 合同表附件下载保持同一授权边界

对象格式化层继续为每个 `files` 元素生成带索引的后端下载 URL。合同表仅把现有数量文本改为逐个链接；下载控制器仍检查对象查看权限和 `ProjectVisibility`，并从存储元数据取得安全原始文件名。没有元数据的历史附件按旧本地路径读取，避免迁移时导致既有合同附件失效。
...
- 模型伪造或扩大导出：导出工具只接受查询规格，实际数据由服务端按发起人权限重新读取并限制规模。
- CLI 命令注入或密钥泄漏：使用 argv 数组和标准输入，不拼接 shell；密钥只用于一次配置，不进入应用参数、日志或测试夹具。
- 创建成功而后续写入失败：表格 shortcut 的部分失败会返回明确错误及已创建资源信息；不自动删除资源，避免二次破坏。
...
- 关闭 `FEISHU_CLI_ENABLED` 即禁用新导出工具；移除 CLI 不影响既有提醒、查询和站内数据。
