...
- 新增中标人工流转：招投标负责人在「已中标」确认时人工选择接手的业务员，系统创建项目主档记录、回写 `converted_project_id` 与 `assignee_user_id`，并通知该业务员与全部管理员；中标后可在招投标表直接改派接手业务员，并同步项目负责人。
...
- 前端：通用本体 UI 自动承接招投标 CRUD；补充中标流转确认对话框、表格内中标后业务员改派与预警展示。
