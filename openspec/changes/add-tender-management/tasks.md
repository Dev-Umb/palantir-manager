...
- [x] 3.6 在 tender 表追加 `assignee_user_id` 接手业务员字段，流转时回写；仅中标且已流转记录允许 tender/admin 在表格或编辑表单改派。
- [x] 3.7 改派在同一事务内同步关联项目 `business_owner_user_id`，通知新接手人并写审计；L2 覆盖成功、未中标、非 business、business 只读边界，L3 覆盖动态可编辑性。
